...
ofstream BenchmarkFile("benchmark_optimized.csv");
...
int main() {
    ...
    for (int N=1; N < 10; N++)
        generateAndMultiplicateOptimized(N*10);
    for (int N=1; N < 10; N++)
        generateAndMultiplicateOptimized(N*100);
    for (int N=1; N < 11; N++)
        generateAndMultiplicateOptimized(N*1000);
    BenchmarkFile.close();
    return 0;
}